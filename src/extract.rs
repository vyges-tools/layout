//! Native device extraction — a GDS layout + extraction rules -> a `Netlist`,
//! using this crate's geometry kernel. Shared foundation used by LVS (device
//! comparison) and any engine that needs devices/nets straight from layout.
//!
...
pub fn extract(lib: &Library, top: Option<&str>, rules: &Rules) -> Result<Netlist, String> {
    ...
    let flat = if has_refs { Some(crate::flatten::flatten(lib, &base.name)?) } else { None };
