...
//!
//! Authored as ASCII art and mechanically converted by `tools/genfont.py`, so a wrong glyph shows
//! up as a visible typo in review rather than as one wrong hex digit nobody can see. Edit the art
//! and regenerate; do not hand-patch the table.

/// 5x7 glyphs, column-major: byte `n` is column `n`, bit 0 the top row.
/// Generated by `tools/genfont.py` — edit the art there, not this table.
pub const FONT: [(char, [u8; 5]); 85] = [
...
pub fn fallback(c: char) -> char {
    match c {
        ...
        '\u{00a9}' => '\u{00a9}',                       // copyright — has its own glyph below
        '\u{03a9}' => 'O',                            // ohm
        other => other,
    }
}
